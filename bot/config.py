...
@dataclass
class MT5Config:
    """MetaTrader5 connection settings - Optimized for Exness"""
    login: int = int(os.getenv("MT5_LOGIN", "0"))
    password: str = os.getenv("MT5_PASSWORD", "")
    # Exness servers: ExnessMarketsPro-Demo, ExnessMarketsPro-Real, etc.
    server: str = os.getenv("MT5_SERVER", "ExnessMarketsPro-Demo")
...
class TradingConfig:
    ...
    # SMA Strategy - Exness optimized for 1-minute timeframe with tight spreads
    fast_ma_period: int = int(os.getenv("FAST_MA_PERIOD", "12"))
    slow_ma_period: int = int(os.getenv("SLOW_MA_PERIOD", "26"))
    ...
    # Risk Management (Global across all pairs) - Conservative for demo
    stop_loss_pips: int = int(os.getenv("STOP_LOSS_PIPS", "30"))
    take_profit_pips: int = int(os.getenv("TAKE_PROFIT_PIPS", "60"))
    max_daily_loss_percent: float = float(os.getenv("MAX_DAILY_LOSS_PERCENT", "3.0"))
    max_position_size: float = float(os.getenv("MAX_POSITION_SIZE", "0.5"))  # Exness allows micro lots
    max_trades_per_day: int = int(os.getenv("MAX_TRADES_PER_DAY", "10"))  # Global limit across all pairs
    ...
    def __post_init__(self):
        ...
        if self._symbols_str:
            self.symbols = [s.strip() for s in self._symbols_str.split(",") if s.strip()]
        else:
            self.symbols = [self._single_symbol]
    ...
    def symbol(self) -> str:
        ...
        return self.symbols[0] if self.symbols else "EURUSD"
